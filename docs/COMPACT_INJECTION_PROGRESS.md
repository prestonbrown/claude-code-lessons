# Compact Approach Injection - Progress & Evaluation

## Problem Statement (2026-01-01)

The approach injection system was dumping ALL tried steps into context, causing:
...
- Completed approaches weren't being archived
...
### 5. Archive Old Completed Approaches
Completed approaches auto-archive after 3 days (APPROACH_COMPLETED_ARCHIVE_DAYS).
...
| **Total per approach** | ~2200 tokens | ~200 tokens | **90%** |
...
3. **Stale work auto-archives** - No more completed approaches sitting in active
4. **Phase stays accurate** - Auto-updates based on actual work done
5. **Work auto-completes** - "Final commit" step marks approach done
...
### Test 2: Complete an approach
Add a tried step starting with "Final":
```
APPROACH UPDATE A002: tried success - Final implementation complete
```
Check:
- [ ] Approach status changes to "completed"
- [ ] Phase changes to "review"

### Test 3: Archive after 3 days
Wait 3+ days after completing an approach, then check:
- [ ] Approach moves to APPROACHES_ARCHIVE.md
- [ ] No longer appears in active injection

## Files Changed

1. `core/models.py` - Added APPROACH_COMPLETED_ARCHIVE_DAYS constant
2. `core/handoffs.py`:
   - Added `_extract_themes()`
   - Added `_summarize_tried_steps()`
   - Added `_archive_old_completed_approaches()`
   - Modified `approach_inject()` for compact format
   - Modified `approach_add_tried()` for auto-complete/phase
3. `tests/test_handoffs.py` - 48 new tests (200 total now)

## Commits

1. `4a6a0b6` - feat(approaches): add auto-complete, auto-phase, and completed archival
2. `1787f7f` - feat(approaches): compact injection format with 90% token reduction
...
- [ ] Consider relevance scoring for which approaches to inject
